...
- `listSpaces()` should do one recursive `fileList("~/spaces/", true)` pass, then batch-read the discovered manifests plus YAML widget files together, keep widget metadata scoped to the concrete folder being listed, collapse duplicate manifests that normalize to the same space id by preferring the canonical folder name, and keep a short-lived in-memory snapshot for direct-route reuse
