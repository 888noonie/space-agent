...
- API-mode request-url resolution should accept either a full chat-completions endpoint or a provider base URL; when the configured endpoint path is just `/` or `/v1`, `api.js` should normalize it to `/chat/completions` or `/v1/chat/completions` before proxy resolution so Ollama-style base URLs do not fail with upstream `405` responses
